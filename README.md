...
In this code, we have examined well-known Ornstein-Uhlenbeck type stochastic volatility model, namely BNS Model, and its uses in option valuation. 

First, we simulated the background driving Lévy process (BDLP), the OU Process through BDLP, and the asset price with synthetic model parameters. This allowed us to demonstrate one of the important properties of BNS model which is the leverage effect.
